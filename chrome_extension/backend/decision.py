...
selecting a tool if needed, and continuing until the FINAL_ANSWER is produced.{tool_context}
...
"""
    logger.loggingInfo(f"[plan] prompt:{prompt}")
    try:
        ...
        for line in raw.splitlines():
            if line.strip().startswith("FUNCTION_CALL:") or line.strip().startswith("FINAL_ANSWER:"):
                return line.strip()
        logger.loggingInfo(f"[plan] LLM output: {raw}")
        return raw.strip()

    except Exception as e:
        logger.loggingInfo(f"[plan] Decision generation failed: {e}")
