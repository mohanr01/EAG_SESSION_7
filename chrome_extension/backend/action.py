...
def parse_function_call(response: str) -> tuple[str, Dict[str, Any]]:
    ...
    try:
        ...
        logger.loggingInfo(f"Parsed: {func_name} → {result}")
        return func_name, result

    except Exception as e:
        logger.loggingError(f" Failed to parse FUNCTION_CALL: {e}")
        raise
...
async def execute_tool(session: ClientSession, tools: list[Any], response: str) -> ToolCallResult:
    ...
    try:
        ...
        logger.loggingInfo(f"Calling '{tool_name}' with: {arguments}")
        ...
        logger.loggingInfo(f"tool: {tool_name} result: {out}")
        ...
        logger.loggingError(f"Execution failed for '{response}': {e}")
        raise
