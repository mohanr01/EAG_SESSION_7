...
Output only the dictionary on a single line. Do NOT wrap it in ```json or other formatting. Ensure `entities` is a list of strings, not a dictionary.
    ...
    try:
        ...
        logger.loggingInfo(f"[perception] LLM output: {raw}")
        ...
        try:
            ...
            logger.loggingInfo(f"[perception] Failed to parse cleaned output: {e}")
            raise
        ...
        logger.loggingInfo(f"[perception] Extraction failed: {e}")
